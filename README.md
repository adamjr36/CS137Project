...
related work was 63.3% in predicting 2018 FIFA World Cup matches
[@rahman2020], which used a recurrent (LSTM) model. It also used match
results back from the 19th century, and they used the FIFA points
ranking system as the major model inputs to make their predictions, not
any detailed data or statistics about teams or players.
...
al [@groll2021]. They try many different kinds of data, like FIFA
rankings, GDP/population of each country, historic match results,
bookmakers data, and plus/minus of each player. Still, they never use
specific game statistics that actually inform match outcomes.
...
$X \in \mathbb{R}^{N \times 2 \times K \times D}$, where $N$ is batch
size, $2$ represents the two teams that play in each match (home and
away), $K$ is length of match history, $D$ is the number of features
($106$ in the case of our model.)
...
model's input, $X \in \mathbb{R}^{1 \times 2 \times 10 \times 106}$, for
each team, includes the last $10$ matches of data before Date $d$.

The input to the Augmented Model includes $X$, as well as
$X' \in \mathbb{R}^{N \times 2 \times L}$, where $N$ still represents
batch size, as $2$ is for the number of teams, and $L$ denotes the
number of headlines from Google News used to encode sentiment for each
team.

The output of the model is $O \in \mathbb{R}^{3}$, representing the
three output classes, Home Win, Draw, or Home Loss.
...
\"Competition\" which was always \"Premier League\",) important columns
were added (\"Win\", \"Score\",) and other important processing was
done, such as mapping the \"Scheme\" column (strings representing team
formation: '4-2-3-1', '5-3-2', etc.) to integers.
...
$X \in \mathbb{R}^{N \times 2 \times K \times D}$. While this takes
longer than it would take to store this data directly, we made this
decision to only store data in $O(M)$ space, where $M$ is the number of
games we have data for, instead of $O(MK)$ space, where $K$ is a
...
implemented with $LSTM$ that maps $R^{N \times K \times D}$ into
$R^{N \times F}$, where $F$ is the dimension of our team feature vector
...
pre-trained GloVe model with 50-dimension [@pennington2014glove],
followed by an LSTM to encode the sentiment of the news about the team.
The encoded sentiment vectors are then fused with the encoded team data
(from the Team Analyzer,) in order to create a rich sentiment
representation of each team, which are then passed to the Predictor with
...
$128$, hidden_size1 = $256$, hidden_size2 = $128$. We used 10 hidden
layers in the Team Analyzer, 12 hidden layers in the Predictor. Later
when switching to a convolutional neural network, we did not achieve
very good results.
...
chronologically, the accuracy fell to $60.1\%$ We then tried replacing
the feedforward network in in the Team Analyzer with a $GRU$, and our
...
with a much higher validation accuracy: $57.2\%$, where previously we
had started off with validation accuracy $20\% - 40\%$. Its testing
accuracy was also highest: $65.2\%$.
...
bibliography: report_src/ref.bib
